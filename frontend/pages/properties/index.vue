<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold tracking-tight">{{ t('navigation.myProperties') }}</h1>
        <p class="mt-2 text-muted-foreground">
          {{ t('property.manageYourProperties') }}
        </p>
      </div>
      <NuxtLink to="/properties/new">
        <Button>
    ...
    <Card v-else-if="!properties.length" class="p-12">
      <div class="text-center">
        ...
        <h3 class="text-lg font-medium">{{ t('property.noProperties') }}</h3>
      ...
      >
        <CardContent class="p-6">
          ...
          <div class="flex items-center gap-4 text-sm text-muted-foreground">
            ...
            <div v-if="property.furnishing_status === 'partially_furnished' || property.furnishing_status === 'fully_furnished'" class="flex items-center gap-1">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div v-if="property.listing && property.listing.monthly_rent && property.listing.currency" class="mt-4 pt-4 border-t border-border">
            <div class="flex items-center justify-between">
...
const loadProperties = async (page = 1) => {
  ...
  try {
    const response = await fetchProperties({ page, per_page: 12, sort: '-created_at' })
